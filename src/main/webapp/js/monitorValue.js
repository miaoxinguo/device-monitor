	...
	if(total>0 && !$('#monitorValueTable tbody tr td').hasClass("dataTables_empty")){
	    ...
	    $("#modal_div_content .row").each(function(rowIndex){
	        ...
	        for(var i=offset ;i<offset+rowSize && i<total; i++){
	        	...
	        	// 图标点击事件
	        	$modalDiv.find("div.panel-footer a:eq(2)").click(function(){
	        		var $reportModal = $("#report_modal").modal();
	        		$reportModal.find(".modal-title").text("设备"+$tr.children("td:eq(1)").text()+"数据曲线");
	        		$reportModal.find(".modal-body").load("html/monitorValueReport.html"); 
	        	})
	        }
	    });
	}
